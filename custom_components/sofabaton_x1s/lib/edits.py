...
from .payloads import IrPayload
...
def set_command_payload(
    bundle: dict[str, Any], device_id: int, command_id: int, payload: IrPayload
) -> dict[str, Any]:
    ...
    engine; only the bytes change. A ``sync_device`` on ``device_id``
    writes it.
    """
...
def add_command(
    bundle: dict[str, Any], device_id: int, payload: IrPayload, name: str,
    *, command_id: Optional[int] = None,
) -> tuple[dict[str, Any], int]:
    """Add a command with ``payload`` to a device; returns ``(edited, command_id)``.

    Without ``command_id`` the next free slot is taken. A ``sync_device``
    on ``device_id`` persists the record (the planner recognises the
    ``restore_data.new`` marker).
    """
