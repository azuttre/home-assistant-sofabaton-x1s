...
| `remote-card/`                         | The remote card's TypeScript source (`src/`): shared by the Home Assistant card (`www/remote-card.js`) and the web remote that sofabaton-x-server serves (`sofabaton-x-server/src/sofabaton_server/ui/remote-web.js`). `src/backend/` holds the backend port and its two adapters, `src/shims/` the elements and palette the page needs instead of Home Assistant's frontend; `shims/palette.ts` and `shims/mdi-icons.ts` are generated by `scripts/build-remote-web-assets.mjs`. |
...
npm run build:remote-web    # remote-card/src/remote-web.ts  -> sofabaton-x-server/src/sofabaton_server/ui/remote-web.js
npm run build:frontend      # all three bundles
