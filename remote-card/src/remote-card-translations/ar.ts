...
export const REMOTE_CARD_STRINGS_AR = {
  ...
  assist: {
    ...
    notification: {
      ...
      eventCommand: (label: string) => `الأمر: ${isolate(label)}`,
      ...
      headerDevice: (deviceName: string, eventLabel: string) =>
        `**الجهاز: ${isolate(deviceName)} • ${isolate(eventLabel)}**`,
  ...
  editor: {
    ...
    longPressDescription:
      "اضغط مطوّلًا على أحد الأزرار المحددة لإرسال أمره بشكل متكرر، كما في جهاز التحكم الفعلي.",
    ...
    shortcutCommandMissing: (id: number | string) =>
      `الأمر ${isolate(id)} (مفقود)`,
    shortcutsCommandsLoading: "جارٍ تحميل الأوامر…",
    shortcutsCommandsUnavailable:
      `أوامر هذا الجهاز غير مخزّنة مؤقتًا بعد. حدِّث الجهاز من تبويب ${isolate("Hub")} في ${isolate("Sofabaton Control Panel")}، ثم أعد تحميل لوحة المعلومات.`,
    shortcutsCommandsError:
      "تعذّر تحميل أوامر هذا الجهاز. أعد تحميل لوحة المعلومات وحاول مجددًا.",
