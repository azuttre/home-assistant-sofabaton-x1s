...
export const REMOTE_CARD_STRINGS_EN = {
  ...
  assist: {
    ...
    notification: {
      ...
      eventCommand: (label: string) => `Command: ${label}`,
  ...
  editor: {
    ...
    tintedPanelsDescription:
      "Show a tinted background behind each group of buttons.",
    ...
    deviceModeDescription:
      "Control one device configured on the hub, using that device's button assignments and complete command list.",
    ...
    shortcutsCommandsError:
      "Could not load this device's commands. Reload the dashboard and try again.",
